...
CONFIG\_LOCK\_DEBUGGING & \textit{n}                   &             \\ \hline
\end{tabular}
\centering
...
\begin{tabular}{|c|c|}
\hline
\textbf{Measurement point} 	& \textbf{Time-stamp}  \\ \hline
1							& 1462886735010040031  \\ \hline
2							& 1462886735010146016  \\ \hline
3							& 1462886735018168837  \\ \hline
4							& 1462886735018233859  \\ \hline
\end{tabular}
\centering
\caption{Raw data}
...
%%%%%%%%%%%%%%%%%%%%%%
\subsection{Independent Variables}\todo{reformulate the stress variable}
...
The deployment context is executing the experimental units on the target system natively or in a Docker container. The execution enviroment is an alternating of the independent variables. Further details on the execution enviroments are specified in \ref{section:exe-env}.
...
The dependent variables are based on the duration between time-points within all executed time-slices using a measurement unit of \texttt{nanoseconds}. The procedure for processing the raw data captured from the experiment machine, is done through a python script. The script calculates the duration between two time-stamps using the equation shown in \ref{eq:calculate-duration} and stores the specific duration into a \textit{.csv} file.
...
To address both research questions and to ensure full coverage, the experiment results is first analysed through an multivariate analysis (MANOVA) to understand whether the different treatments have an overall impact on all of the dependent variables. Where the collection of all dependent variables reflects the overall scheduling precision of the execution environment. To better understand the impact of the treatments the coefficient derived corresponding to each of the dependent variables is analysed.\\
