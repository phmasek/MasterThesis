...
There exists groups of dependent variables which are all impacted by switching treatment. It is important to look both at the aggregated view of the variables, as well as at each of the variables independently. The sets of the dependent variables are namely; 1) \textit{scheduling precision}, 2) \textit{camera IO}, and 3) \textit{disk IO}. \textit{Scheduling precision} refers to how accurately, in terms of time, the system performs the executions within the time-slice. Where the data points are the duration between four measurement points for the Pi Component.\\
...
The treatments that will be used for assessing the impact are the factors mentioned earlier, namely: 1) Kernel (vanilla and RT\_PREEMPT patched kernel) and 2) Deployment context (Docker or natively). These variables will be the variables upon which the research questions will be answered. By analysing how each of the independent variables affect the dependent variables during two different types of load.
...
To address both research questions and to ensure full coverage, the experiment results will be analysed through an analysis of variance (ANOVA) to understand the impact the different treatments have on the dependent variable.\\\todo{Write about the python code and how the data was processed for the analysis}
